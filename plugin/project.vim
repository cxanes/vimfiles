...
function! s:Project(filename) " <<<
    ...
    if exists("g:proj_running")
        ...
        if !exists('g:proj_filename')
            let g:proj_filename = '.vimproject' " Default local project filename
        endif
        if strlen(a:filename) == 0
            if filereadable(g:proj_filename)
                let filename = g:proj_filename
            else
    ...
    if !exists('g:proj_foldcolumn') || type(g:proj_foldcolumn) != type(0)
        let g:proj_foldcolumn = 0               " Project Window fold column width
    endif
