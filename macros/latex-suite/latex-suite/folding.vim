...
function! TexFoldTextFunction()
	...
	elseif getline(v:foldstart) =~ '^\s*\\document\(class\|style\).*{'
		let ftxto = leadingSpace.s:Foldtext()
		return substitute(ftxto, ':', ': Preamble: ', '')
	else
		return leadingSpace.s:Foldtext()
	end
endfunction
function! s:Foldtext()
	let ftxto = substitute(foldtext(), '^+-*\s*\d\+\s\+lines:\s\+', '', '')
	if v:foldlevel > 1 && indent(v:foldstart) == 0
		let ftxto = repeat(' ', (v:foldlevel-1)*2) . ftxto
	endif
	return ftxto
endfunction
" }}}
