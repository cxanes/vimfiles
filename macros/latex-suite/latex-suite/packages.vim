...
function! Tex_pack_check(package)
	...
	if dictname != ''
		exe 'setlocal dict^='. substitute(dictname, '\([ ,\\]\)', '\\\\\\\1', 'g')
...
function! Tex_pack_updateall(force)
	...
	" Sometimes closing the current window don't go back to the previous
	" window which we split.
	if bufwinnr(cur_buffer) != -1
		exe bufwinnr(cur_buffer) . 'wincmd w'
	endif
